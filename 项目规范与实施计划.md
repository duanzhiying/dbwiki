...
### 6. 常见问题
#### 端口占用与释放（macOS / zsh）
说明：本项目在本地使用 `mkdocs serve` 或其他开发服务器时，常遇到端口被占用导致启动失败。下面汇总常用命令（可直接复制粘贴），把 8080 替换为你需要检查的端口号。

- 查看占用指定端口的进程（推荐）

```bash
# 显示占用端口的进程与端口信息（需要 sudo 可选）
sudo lsof -nP -iTCP:8080 -sTCP:LISTEN

# 仅输出占用该端口的 PID（便于配合 kill）
...
```bash
# 使用 npm 一次性安装并运行 kill-port（不全局安装）
npx kill-port 8080
```

提示：
- 尽量先用 `kill PID`（默认发送 SIGTERM，允许进程清理退出），只有在无效时再使用 `kill -9`（SIGKILL）。
- 所有命令中的 `8080` 为示例端口，请替换为实际端口号。
- 如果频繁遇到端口冲突，考虑使用 `mkdocs serve --dev-addr=127.0.0.1:xxxx` 指定不同端口。
